...
class StateEvent:
    ...
    reason: str
    created_at: str
    # Провенанс: id шага, в рамках исполнения которого применён патч (для
    # ролбека — селективный реплей переживших патчей). None — патч вне шага
    # (инициализация / решение пользователя / глобальное действие): такие
    # всегда сохраняются при откате.
    task_id: str | None = None
    # Монотонный порядковый номер (autoincrement id в БД) — устойчивый порядок
    # реплея. Заполняется при чтении из лога.
    seq: int | None = None
    # Ролбек: id отката, который аннулировал этот патч (NULL = активен). При
    # реконструкции аннулированные патчи не переигрываются.
