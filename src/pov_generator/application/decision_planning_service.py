...
class DecisionPlanningService:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "- МЕТА-решения о процессе генерации, оформлении артефакта, "
            "формате/структуре документа, шаблоне, нумерации разделов, "
            "стиле изложения, способе оформления цитат / списков / таблиц;\n"
            "- решения «как мы будем работать с blocking_questions / "
            "вопросами / уточнениями / реестром / checkpoint» — это "
            "процессная механика самой системы, она к содержанию проекта "
            "не относится;\n"
            "- решения «включить ли раздел X в этот артефакт», «нужен ли "
            "блок Y», «формат таблицы Z» — это шаблон, не проект.\n\n"
            "В реестр должны попадать ТОЛЬКО решения о содержании проекта "
            "PoV (что мы строим, для кого, как технически устроено, какие "
            "trade-off закладываем). Если решение касается того, как "
            "выглядит документ или как работает система генерации — "
            "пропускай.\n"
            "</what_to_skip>\n\n"
    ...
    def _build_single_decision(
        ...
        *,
        ...
        if len(alternatives) < 2:
            # Меньше двух реальных альтернатив = это не решение, а готовый
