    ...
    def _build_prompt(
        ...
        *,
        ...
        system_prompt = (
            ...
            "12. ОПИСАНИЕ, НЕ ОБЯЗАТЕЛЬСТВО. Описывай решение и его результаты "
            "как факты дизайна: «решение производит X», «PoV включает Y». Не пиши "
            "языком обязательств и гарантий («команда обязуется поставить», "
            "«гарантируется», «подрядчик предоставляет») — и не оговаривай, что "
            "гарантий нет.\n\n"

            "13. РОЛИ, НЕ ИМЕНА. Участников, стейкхолдеров и ответственных "
            "обозначай ролью и зоной ответственности («владелец процесса», "
            "«владелец данных», «аналитик качества»), а не личным именем — даже "
            "если ФИО присутствует в источнике.\n"
            "</writing_principles>\n\n"
            ...
            "<out_of_scope>\n"
            "Следующие темы — вне этого документа (их решают вне системы). НЕ "
            "выводи и не упоминай их, и НЕ пиши, что их здесь нет, — просто не "
            "затрагивай: стоимость, бюджет, цены; сроки, даты, длительности, "
            "календарь; юридические условия — права на ИС, гарантии, поддержка, "
            "обязательства сторон. Этапы и фазы, если уместны, описывай как "
            "порядок работ с зависимостями, без длительностей и дат.\n"
            ...
            "придумывай: опусти или пометь как требующее уточнения.\n"
            "</out_of_scope>\n\n"
        ...
        *,
        ...
        if artifact_role == "requirements_spec":
            ...
            spec = {
                ...
                # acceptance_criteria — массив ОБЪЕКТОВ {criterion, verification_method}
                # (см. контракт). Заворачиваем строковые критерии из upstream-приёмки
                # в объекты; пусто — два дефолтных объекта.
                "acceptance_criteria": [
                    {
                        "criterion": str(item.get("criterion") or item) if isinstance(item, dict) else str(item),
                        "verification_method": (
                            str(item.get("verification_method"))
                            if isinstance(item, dict) and item.get("verification_method")
                            else "Проверяется обзором результата со стороны заказчика на приёмке"
                        ),
                    }
                    for item in (
                        acceptance.get("acceptance_criteria", []) if isinstance(acceptance, dict) else []
                    )
                ]
                or [
                    {
                        "criterion": "Документ содержит цели, требования, ограничения и критерии приёмки",
                        "verification_method": "Проверяется обзором документа со стороны заказчика",
                    },
                    {
                        "criterion": "ТЗ пригодно для передачи в проектирование и реализацию",
                        "verification_method": "Подтверждается командой реализации без критических догадок",
                    },
                ],
