...
# Порог ПРОАКТИВНОЙ декомпозиции. Компромисс между двумя издержками этого CLI:
# мелкое дробление → МНОГО вызовов (каждый заново шлёт весь промпт → раздувает
# cache-токены и упирается в rate-limit подписки), а слишком крупные схемы →
# strict-штормы. Подписка штрафует именно за объём вызовов, поэтому порог
# держим умеренно ВЫСОКИМ: средние схемы идём одним проходом (при неудаче
# деградируем на plain — это 1 вызов, а не N батчей), и дробим лишь заведомо
# тяжёлые. Переопределяется POV_COMPOSITIONAL_THRESHOLD.
