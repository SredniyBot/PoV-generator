...
# Порог ПРОАКТИВНОЙ декомпозиции. Низкий намеренно: реактивный путь НЕ успевает
# предотвратить strict-шторм — многоходовая coercion случается ВНУТРИ одного
# inner-вызова, ДО того как мы увидим невалидный результат. Значит всё, что
# склонно штормить, надо дробить ЗАРАНЕЕ. Эмпирика реальных прогонов: схемы
# cx≤5 (constraint/goal, ~6 полей) strict берёт одним ходом, на cx≥8 уже штормит.
# Порог 6: простые схемы идут одним проходом, остальное — дробится на простые
# батчи (см. _SCALAR_GROUP_BUDGET в decomposer). Переопределяется
# POV_COMPOSITIONAL_THRESHOLD.
