...
- `TemplateRecipeFragment` — доменное расширение базового recipe;
- `DomainPack` — подключаемый пакет доменных знаний;
...
- Хранит **recipe fragments** — доменные расширения базовых сценариев
- Хранит **domain packs** — декларативные контейнеры доменных vocabulary, templates и recipe fragments
...
| **Planning Coordinator** | читает `template` + `recipe` + `recipe fragment` + `domain pack` + vocab entries; использует registry как source of truth для admissibility/orchestration objects |
...
Recipe — это base orchestration object. Для доменных расширений registry обязан хранить ещё и `TemplateRecipeFragment`.

### 2.8.1. `TemplateRecipeFragment`

Recipe fragment нужен, чтобы новый домен влиял на уже существующий класс задач без переписывания base recipe.

```python
class RecipeInsertionPoint(BaseModel):
    anchor_step_id: str
    position: Literal["before", "after", "replace"]

class TemplateRecipeFragment(BaseModel):
    fragment_id: NamespacedId
    version: TemplateVersion
    description: str
    status: Literal["draft", "active", "deprecated"]
    target_recipe_ids: list[NamespacedId]
    domain: str
    activation_signals: list[NamespacedId] = []
    insertion_points: list[RecipeInsertionPoint]
    mandatory_steps: list[RecipeStep]
    adds_readiness: list[NamespacedId] = []
    adds_artifact_roles: list[NamespacedId] = []
    provenance: Provenance
```

Нормативные правила:

- fragment не содержит `core_template_refs`; он расширяет существующий сценарий;
- fragment обязан явно указать, в какой base recipe он может быть встроен;
- fragment не может использовать `latest` для ссылок на шаблоны;
- validator обязан проверять, что `anchor_step_id` существует в target recipe.

### 2.8.2. `DomainPack`

Domain Pack — first-class declarative object, который собирает доменные знания в одну подключаемую единицу.
...
- Domain Pack не заменяет templates/recipes; он связывает их в доменное расширение;
- pack может расширять построение ТЗ, архитектуры и другие сценарии через `recipe_fragment_refs`;
- planner не должен содержать hard-coded knowledge о конкретном домене; он видит только подключённые packs и их декларативные объекты.
...
├── recipe_fragments/
│   └── <domain>/<fragment_id>@<version>.yaml
...
CREATE TABLE recipe_fragment_index (
...
CREATE TABLE domain_pack_index (
...
| V20 | `recipe_fragment.target_recipe_ids[*]` существуют | `RecipeCompatibilityError` |
| V21 | `recipe_fragment.insertion_points[*].anchor_step_id` существует в target recipe | `RecipeCompatibilityError` |
| V22 | `domain_pack.recipe_fragment_refs[*]` и `template_refs[*]` существуют | `RecipeCompatibilityError` |
| V23 | `domain_pack.domain` совместим с domains всех referenced objects | `RecipeCompatibilityError` |
| V24 | `domain_pack.entry_signals[*]` и `readiness_types[*]` существуют в vocabulary | `UnknownVocabularyEntryError` |
...
class TemplateRegistry(Protocol):
    ...
    def load_recipe_fragment(self, fragment_id: str, version: TemplateVersion) -> TemplateRecipeFragment: ...
    def load_domain_pack(self, pack_id: str, version: TemplateVersion) -> DomainPack: ...
    def list_recipes(self, *, domain: str | None = None, status: str | None = None) -> list[RecipeSummary]: ...
    def list_recipe_fragments(self, *, domain: str | None = None, status: str | None = None) -> list[RecipeFragmentSummary]: ...
    def list_domain_packs(self, *, domain: str | None = None, status: str | None = None) -> list[DomainPackSummary]: ...
...
class RecipeFragmentSummary(BaseModel):
    fragment_id: str
    version: TemplateVersion
    status: str
    domain: str
...
1. Разработчик создаёт или обновляет:
   ...
   - при необходимости `recipe_fragments/<domain>/<fragment_id>@<version>.yaml`,
   - при необходимости `domain_packs/<domain>/<pack_id>@<version>.yaml`,
...
### 6.3.1. Публикация нового доменного пакета

1. Разработчик создаёт:
   - `domain_packs/<domain>/<pack_id>@<version>.yaml`,
   - при необходимости `recipe_fragments/<domain>/<fragment_id>@<version>.yaml`,
   - новые templates и vocabulary entries домена.
2. CI проверяет:
   - что pack резолвит все ссылки;
   - что fragment корректно встраивается в target recipes;
   - что новые vocabulary ids не конфликтуют с существующими.
3. После публикации pack становится доступным Recipe Composer'у и project bootstrap layer.
...
- Валидатор: по одному тесту на каждое правило V1–V24 (позитив + негатив).
...
- Для каждого recipe fragment и domain pack — снимок результата `model_dump()`.
...
- Интеграционные проверки compatibility graph: template↔recipe↔fragment↔domain_pack↔vocabulary.
