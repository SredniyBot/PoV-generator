...
- **RG-C** — РЕАЛЬНЫЙ harvest бандла из песочницы: дерево кода из зоны сервиса
  (`harvest_path` из `build_manifest.services[].zone`); каркас/интеграция —
  весь `/work`; `.povgen` исключается, пути относительны. aider — diff-harvest.
- **RG-E** — полиморфный веер: дочерний шаблон по рецепту капабилити элемента
  (`FanOutSpec.recipe_field` → `capability_profile.build_recipe`), защитный
  fallback на общий child; validate резолвит рецепт.
- **RG-F** — рецепты сборки по капабилити для всех типов (UI/backend/ML/data/
  integration): специализированный бриф компонента, та же роль
  component_implementation. Новый тип сервиса = capability + build_recipe + лист.

Осталось (опционально):
- **RG-E (scoped fan-out)** — рекурсия до модуля: веер по модулям компонента
  (источник, ограниченный фокусом родителя) + per-component internal contract.
  Адаптивный «модульный пол» — только для тяжёлых компонентов. Большая правка
  планировщика; по умолчанию модули — build_steps внутри одного прогона.

Зависимость: реальная генерация кода (claude-harness) требует прогона на
Docker/claude (среда пользователя); граф и harvest на stub-пути зелёные.
